...
#define IOCTL_RESET_BUFFER _IO(IOCTL_MAGIC, 1)

static void print_usage(const char *prog)
{
    fprintf(stdout, "Usage: %s [-hdwri]\n", prog);
    fprintf(stdout, "\t-d --device\t\t\t\t: device to use.\n");
    fprintf(stdout, "\t-w --write\t\t\t\t: write a new message.\n");
    fprintf(stdout, "\t-r --read\t\t\t\t: read the message managed by char device.\n");
    fprintf(stdout, "\t-i --ioctl\t\t\t\t: perform ioctl operations.\n");
...
int main(int argc, char* argv[])
{
    ...
    do {
        c = getopt_long(argc, argv, "hrid:w:", lopts, &option_index);
        if (c == -1)
            break;
        ...
        switch(c) {
            ...
            case 'i':
                fprintf(stdout, "Performing IOCTL operation (reset buffer)...\n");
                ret = ioctl(fd, IOCTL_RESET_BUFFER);
                if (ret < 0) {
                    fprintf(stderr, "ioctl: %s\n", strerror(errno));
                    close(fd);
                    return -1;
                }
                fprintf(stdout, "Buffer reset successful\n");
                break;
